...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['content-creator', 'administrator', 'channel-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<MeInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/us` : null),
    () => getUs({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Me',
      path: isTenantUser ? `/us/view/${data?.[0]?.id}` : '/us',
      entity: 'me',
